...
class CategoryAdapter(
    ...
    override fun areContentsTheSame(
        ...
        return oldItem.getOrNull(0) == newItem.getOrNull(0)
    }

    override fun getChangePayload(oldItem: List<PostListViewModel.AppInfoDataItem>, newItem: List<PostListViewModel.AppInfoDataItem>): Any? {
        val payloads = ArrayList<Any>()

        if (oldItem.getOrNull(0) != newItem.getOrNull(0)) {
            payloads.add(PAYLOAD_ITEM_RECENT)
        }
    ...
    override fun bind(view: View, viewType: Int, position: Int, item: List<PostListViewModel.AppInfoDataItem>, payloads: MutableList<Any>) {
        super.bind(view, viewType, position, item, payloads)
        if (payloads.contains(PAYLOAD_ITEM_RECENT)) {
            if (item.isNotEmpty()) {
                refreshGroup(view, position, item)
            }
        }
    ...
    private fun refreshGroup(view: View, position: Int, item: List<PostListViewModel.AppInfoDataItem>) {
        var itemApp = item[0]
        Glide
                .with(view)
    ...
    companion object {
        const val PAYLOAD_ITEM_RECENT = "PAYLOAD_ITEM_RECENT"
    }
}
